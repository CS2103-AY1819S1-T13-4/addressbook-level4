...
public class VersionedAddressBookTest {
    ...
    private final ReadOnlyTranscript addressBookWithAmy = new AddressBookBuilder().withPerson(AMY).build();
    private final ReadOnlyTranscript addressBookWithBob = new AddressBookBuilder().withPerson(BOB).build();
    private final ReadOnlyTranscript addressBookWithCarl = new AddressBookBuilder().withPerson(CARL).build();
    private final ReadOnlyTranscript emptyAddressBook = new AddressBookBuilder().build();

    @Test
    public void commit_singleAddressBook_noStatesRemovedCurrentStateSaved() {
        VersionedPredictiveTranscript versionedAddressBook = prepareAddressBookList(emptyAddressBook);
    ...
    public void commit_multipleAddressBookPointerAtEndOfStateList_noStatesRemovedCurrentStateSaved() {
        VersionedPredictiveTranscript versionedAddressBook = prepareAddressBookList(
    ...
    public void undo_singleAddressBook_throwsNoUndoableStateException() {
        ...
        assertThrows(VersionedPredictiveTranscript.NoUndoableStateException.class, versionedAddressBook::undo);
    }
    ...
    public void redo_singleAddressBook_throwsNoRedoableStateException() {
        ...
        assertThrows(VersionedPredictiveTranscript.NoRedoableStateException.class, versionedAddressBook::redo);
    }
    ...
    @Test
    public void equals() {
        VersionedPredictiveTranscript versionedAddressBook = prepareAddressBookList(addressBookWithAmy, addressBookWithBob);

        // same values -> returns true
        VersionedPredictiveTranscript copy = prepareAddressBookList(addressBookWithAmy, addressBookWithBob);
        ...
        VersionedPredictiveTranscript differentAddressBookList = prepareAddressBookList(addressBookWithBob, addressBookWithCarl);
        assertFalse(versionedAddressBook.equals(differentAddressBookList));

        // different current pointer index -> returns false
        VersionedPredictiveTranscript differentCurrentStatePointer = prepareAddressBookList(
     ...
     */
    private void assertAddressBookListStatus(VersionedPredictiveTranscript versionedAddressBook,
                                             List<ReadOnlyTranscript> expectedStatesBeforePointer,
                                             ReadOnlyTranscript expectedCurrentState,
                                             List<ReadOnlyTranscript> expectedStatesAfterPointer) {
        // check state currently pointing at is correct
        assertEquals(new PredictiveTranscript(versionedAddressBook), expectedCurrentState);
        ...
        for (ReadOnlyTranscript expectedAddressBook : expectedStatesBeforePointer) {
            assertEquals(expectedAddressBook, new PredictiveTranscript(versionedAddressBook));
            versionedAddressBook.redo();
        }

        // check states after pointer are correct
        for (ReadOnlyTranscript expectedAddressBook : expectedStatesAfterPointer) {
            versionedAddressBook.redo();
            assertEquals(expectedAddressBook, new PredictiveTranscript(versionedAddressBook));
        }
     ...
     */
    private VersionedPredictiveTranscript prepareAddressBookList(ReadOnlyTranscript... addressBookStates) {
        assertFalse(addressBookStates.length == 0);

        VersionedPredictiveTranscript versionedAddressBook = new VersionedPredictiveTranscript(addressBookStates[0]);
     ...
     */
    private void shiftCurrentStatePointerLeftwards(VersionedPredictiveTranscript versionedAddressBook, int count) {
